...
def test_build_initialize_disk_script() -> None:
    ...
    assert_contains(script, "Clear-Disk -Number 2")
    assert_contains(script, "$diskAfterClear = Get-Disk -Number 2")
    assert_contains(script, "if ($diskAfterClear.PartitionStyle -eq 'RAW')")
    assert_contains(script, "elseif ($diskAfterClear.PartitionStyle -ne 'GPT')")
    assert_contains(script, "Initialize-Disk -Number 2 -PartitionStyle GPT")
    assert_contains(script, "Update-Disk -Number 2")
