    ...
    for disk_number in disk_numbers:
        ...
        lines.extend(
            [
                ...
                "}",
                f"Get-Partition -DiskNumber {disk_number} -ErrorAction SilentlyContinue | Remove-Partition -Confirm:$false",
                "Start-Sleep -Seconds 1",
                f"Clear-Disk -Number {disk_number} -RemoveData -RemoveOEM -Confirm:$false",
                "Start-Sleep -Seconds 1",
                f"$diskAfterClear = Get-Disk -Number {disk_number}",
                "if ($diskAfterClear.PartitionStyle -ne 'GPT') {",
                f"    Initialize-Disk -Number {disk_number} -PartitionStyle GPT",
                "    Start-Sleep -Seconds 1",
                f"    $diskAfterInit = Get-Disk -Number {disk_number}",
                "    if ($diskAfterInit.PartitionStyle -ne 'GPT') {",
                f"        Clear-Disk -Number {disk_number} -RemoveData -RemoveOEM -Confirm:$false",
                "        Start-Sleep -Seconds 1",
                f"        Initialize-Disk -Number {disk_number} -PartitionStyle GPT",
                "        Start-Sleep -Seconds 1",
