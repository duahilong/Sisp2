    ...
    for disk_number in disk_numbers:
        ...
        lines.extend(
            [
                ...
                "$efiOriginalSize = $efiPartition.Size",
                "if ($msrPartition) {",
                "    Remove-Partition -DiskNumber {disk_number} -PartitionNumber $msrPartition.PartitionNumber -Confirm:$false",
                "    Start-Sleep -Seconds 1",
                "}",
                "Remove-Partition -DiskNumber {disk_number} -PartitionNumber $efiPartition.PartitionNumber -Confirm:$false",
                "Start-Sleep -Seconds 1",
                "$newEfiSize = $msrSize + $efiOriginalSize",
                f"$newEfiPartition = New-Partition -DiskNumber {disk_number} -Size $newEfiSize -GptType '{{c12a7328-f81f-11d2-ba4b-00a0c93ec93b}}'",
                "Start-Sleep -Seconds 1",
                "$newEfiVolume = $newEfiPartition | Format-Volume -FileSystem FAT32 -NewFileSystemLabel 'EFI' -Confirm:$false",
                "Start-Sleep -Seconds 1",
                "$results += [PSCustomObject]@{",
                f"    disk_number = {disk_number}",
                "    efi_partition_number = $newEfiPartition.PartitionNumber",
                "    efi_size_bytes = $newEfiPartition.Size",
                "    efi_file_system = $newEfiVolume.FileSystem",
